...
def write_transaction_data_to_csv(data, filename):
...
def write_throughput_data_to_csv(data, filename):
    with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Gas Price (gwei)', 'Throughput (transactions per second)'])
        for row in data:
            writer.writerow(row)
...
def main():
    ...
    for gas_price in gas_prices:
        ...
        write_transaction_data_to_csv(transactions_data, f'transaction_data_{gas_price}gwei.csv')

    write_throughput_data_to_csv(throughput_data, 'throughput_data.csv')
