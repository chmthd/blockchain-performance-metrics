...
def save_to_csv(finality_times):
    with open('finality_times.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Transaction Number', 'Confirmation Time (s)'])
        for i, time in enumerate(finality_times, 1):
            writer.writerow([i, time])

def main():
    num_transactions = 20 
    finality_times = collect_finality_times(num_transactions)
    save_to_csv(finality_times)
