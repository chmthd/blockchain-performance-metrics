...
def write_to_csv(all_data, filename='transaction_throughput_data.csv'):
    with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Offered Load', 'Throughput', 'Transaction Hash', 'Timestamp Sent', 'Timestamp Confirmed'])
        for data in all_data:
            for tx in data['transactions']:
                writer.writerow([data['load'], data['throughput'], tx[0], tx[1], tx[2]])
...
def main():
    ...
    for load in offered_loads:
        ...
        all_data.append({'load': load, 'throughput': throughput, 'transactions': transactions_data})
        print(f"Offered Load: {load}, Throughput: {throughput} transactions per second")

    write_to_csv(all_data)

    # Plotting the throughput vs offered loads
    throughputs = [data['throughput'] for data in all_data]
    plt.figure(figsize=(12, 7))
    plt.plot(offered_loads, throughputs, 'o-', color='#6e57d2', label='Throughput vs Offered Load')
    plt.title('Impact of Load Variation on Transaction Throughput in a 9-Node PoS Network')
